      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg max-w-md mx-auto">
            No hidden fees. Just flat rates you can count on.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          {plans.map((plan, i) => (
            ...
            >
              ...
              <p className={`text-sm mb-6 ${plan.featured ? "text-secondary-foreground/60" : "text-muted-foreground"}`}>
                {plan.description}
              </p>
              <div className="flex items-baseline gap-1 mb-8">
                <span className="font-display font-bold text-4xl">{plan.price}</span>
                <span className={`text-sm ${plan.featured ? "text-secondary-foreground/50" : "text-muted-foreground"}`}>
                  {plan.currency}
                </span>
              </div>
